...
class TestEnsureOllama:
    ...
    def test_missing_model_names_the_pull_command_and_what_is_installed(self):
    ...
    def test_unreachable_server_is_only_a_warning_by_default(self):
        """Diffusion works without a model server, so the stack is degraded, not broken."""
        report = Report()
        with patch.object(start_stack, "ollama_models", return_value=None):
            assert start_stack.ensure_ollama(report, start=False) is False
        assert report.worst() == WARN

    def test_unreachable_server_is_a_failure_when_required(self):
        report = Report()
        with patch.object(start_stack, "ollama_models", return_value=None):
            assert start_stack.ensure_ollama(report, start=False, required=True) is False
    ...
    def test_a_start_attempt_that_never_answers_is_not_counted_as_success(self):
        """
        "starting X" is progress, not evidence. Recording it as ok let a component
        that never came up contribute a passing row.
        """
        report = Report()
        with patch.object(start_stack, "ollama_models", return_value=None), \
             patch.object(start_stack, "start_ollama", return_value="/tmp/ollama.log"), \
             patch.object(start_stack, "wait_until", return_value=None):
            assert start_stack.ensure_ollama(report, start=True, required=True) is False
        assert report.worst() == FAIL
...
class TestExitCode:
    ...
    These drive the real ensure_ollama rather than stubbing it. Stubbing it hid a
    bug where a missing model server always recorded FAIL, so the default run
    exited 1 while printing that the stack was merely degraded.
    """

    def _run(self, argv, health, models):
        with patch.object(start_stack, "check_diffusion", return_value=OK), \
             patch.object(start_stack, "ollama_models", return_value=models), \
    ...
    def test_zero_when_everything_is_up(self):
        ...
        assert self._run(["--no-browser", "--check"], health, ["gemma2:2b"]) == 0

    def test_zero_when_only_chat_is_down_by_default(self):
        """Degraded, not broken: the exit code must agree with the summary text."""
        health = {"gemma_status": {"available": False, "status": "fallback_mode"}}
        assert self._run(["--no-browser", "--check"], health, None) == 0

    def test_nonzero_when_chat_is_down_under_strict(self):
        health = {"gemma_status": {"available": False, "status": "fallback_mode"}}
        assert self._run(["--no-browser", "--check", "--strict"], health, None) == 1

    def test_nonzero_when_the_model_is_not_pulled_under_strict(self):
        health = {"gemma_status": {"available": False, "status": "fallback_mode"}}
        assert self._run(["--no-browser", "--check", "--strict"], health, ["llama3:8b"]) == 1

    def test_nonzero_when_the_api_never_came_up(self):
        assert self._run(["--no-browser", "--check"], None, ["gemma2:2b"]) == 1
